...
import { configureGatewayForSetup, type ConfigureGatewayDeps } from "./setup.gateway-config.js";

// The token generator boundary is injected so this suite does not mock the
// onboard-helpers module at module level (Bun cannot intercept ESM imports).
let generatedToken = "generated-token";
const configureDeps: ConfigureGatewayDeps = { randomToken: () => generatedToken };
...
describe("configureGatewayForSetup", () => {
  ...
  it("prefers DENNOU_GATEWAY_TOKEN during quickstart token setup", async () => {
    const prevToken = process.env.DENNOU_GATEWAY_TOKEN;
    process.env.DENNOU_GATEWAY_TOKEN = "token-from-env";
    generatedToken = "generated-token";
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.DENNOU_GATEWAY_TOKEN = prevToken;
      }
    }
  });
  ...
  it("honors secretInputMode=ref for gateway password prompts", async () => {
    const previous = process.env.DENNOU_GATEWAY_PASSWORD;
    process.env.DENNOU_GATEWAY_PASSWORD = "gateway-secret"; // pragma: allowlist secret
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "password", "off", "env"],
        textQueue: ["18789", "DENNOU_GATEWAY_PASSWORD"],
      });
      const runtime = createRuntime();
      ...
      if (previous === undefined) {
        ...
        process.env.DENNOU_GATEWAY_PASSWORD = previous;
      }
    }
  });

  it("stores gateway token as SecretRef when secretInputMode=ref", async () => {
    const previous = process.env.DENNOU_GATEWAY_TOKEN;
    process.env.DENNOU_GATEWAY_TOKEN = "token-from-env";
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "token", "off", "env"],
        textQueue: ["18789", "DENNOU_GATEWAY_TOKEN"],
      });
      const runtime = createRuntime();
      ...
      if (previous === undefined) {
        ...
        process.env.DENNOU_GATEWAY_TOKEN = previous;
      }
    }
  });
