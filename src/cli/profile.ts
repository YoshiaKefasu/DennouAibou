...
export function applyCliProfileEnv(params: {
  ...
  env.DENNOU_PROFILE = profile;

  const stateDir = env.DENNOU_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.DENNOU_STATE_DIR?.trim()) {
    env.DENNOU_STATE_DIR = stateDir;
  }

  if (!env.DENNOU_CONFIG_PATH?.trim()) {
    env.DENNOU_CONFIG_PATH = path.join(stateDir, "dennou-aibou.json");
  }

  if (profile === "dev" && !env.DENNOU_GATEWAY_PORT?.trim()) {
    env.DENNOU_GATEWAY_PORT = "19001";
  }
}
