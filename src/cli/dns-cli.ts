...
export function registerDnsCli(program: Command) {
  ...
  dns
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log(
        theme.heading(
          "Recommended config ($DENNOU_CONFIG_PATH, default ~/.dennou-aibou/dennou-aibou.json):",
        ),
      );
      ...
      if (cfg.discovery?.wideArea?.enabled !== true) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: enable discovery.wideArea.enabled in the active OpenClaw config ($DENNOU_CONFIG_PATH, default ~/.dennou-aibou/dennou-aibou.json) on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
