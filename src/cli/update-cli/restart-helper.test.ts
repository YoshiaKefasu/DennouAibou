...
describe("restart-helper", () => {
  ...
  describe("prepareRestartScript", () => {
    ...
    it("uses DENNOU_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        DENNOU_PROFILE: "default",
        DENNOU_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("uses DENNOU_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        DENNOU_PROFILE: "default",
        DENNOU_LAUNCHD_LABEL: "com.custom.openclaw",
      });
    ...
    it("uses DENNOU_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        DENNOU_LAUNCHD_LABEL: "ai.openclaw.it's-a-test",
      });
