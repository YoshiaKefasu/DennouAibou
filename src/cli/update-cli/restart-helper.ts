...
function resolveSystemdUnit(env: NodeJS.ProcessEnv): string {
  const override = env.DENNOU_SYSTEMD_UNIT?.trim();
  if (override) {
    return override.endsWith(".service") ? override : `${override}.service`;
  }
  return `${resolveGatewaySystemdServiceName(env.DENNOU_PROFILE)}.service`;
}

function resolveLaunchdLabel(env: NodeJS.ProcessEnv): string {
  const override = env.DENNOU_LAUNCHD_LABEL?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayLaunchAgentLabel(env.DENNOU_PROFILE);
}
...
function resolveWindowsTaskName(env: NodeJS.ProcessEnv): string {
  ...
  return resolveGatewayWindowsTaskName(env.DENNOU_PROFILE);
}
