...
describe("update-cli", () => {
  ...
  it.each([
    ...
    {
      name: "DENNOU_UPDATE_PACKAGE_SPEC override",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("openclaw-update"));
        await withEnvAsync(
          { DENNOU_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/openclaw-next.tgz" },
          async () => {
        ...
        await withEnvAsync(
          {
            ...
            DENNOU_CONFIG_PATH: "./config/dennou-aibou.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedOptions: (root: string) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            DENNOU_STATE_DIR: path.resolve("./state"),
            DENNOU_CONFIG_PATH: path.resolve("./config/dennou-aibou.json"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
    ...
    },
    {
      ...
      expectedOptions: (_root: string, context?: { originalCwd: string }) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            DENNOU_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ DENNOU_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.DENNOU_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    ...
    await withEnvAsync({ DENNOU_GIT_DIR: tempDir }, async () => {
      setTty(true);
  ...
  it("uses ~/openclaw as the default dev checkout directory", async () => {
    ...
    await withEnvAsync({ DENNOU_GIT_DIR: undefined }, async () => {
