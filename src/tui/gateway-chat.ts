...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set DENNOU_GATEWAY_TOKEN/DENNOU_GATEWAY_PASSWORD, pass --token/--password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.DENNOU_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.DENNOU_GATEWAY_PASSWORD);
