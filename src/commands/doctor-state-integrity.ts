...
export function formatLinuxSdBackedStateDirWarning(
  ...
  return [
    ...
    "- For better startup and state durability, prefer SSD/NVMe (or USB SSD on Raspberry Pi) for DENNOU_STATE_DIR.",
  ].join("\n");
}
...
export function detectMacCloudSyncedStateDir(
  ...
  // DENNOU_HOME can relocate app data defaults, but iCloud/CloudStorage remain under the OS home.
  ...
  if (cloudSyncedStateDir) {
    warnings.push(
      [
        ...
        "- Prefer a local non-synced state dir (for example: ~/.dennou-aibou).",
        `  Set locally: DENNOU_STATE_DIR=~/.dennou-aibou ${formatCliCommand("openclaw doctor")}`,
      ].join("\n"),
    );
  }
...
export function noteWorkspaceBackupTip(workspaceDir: string) {
  ...
  note(
    [
      ...
      "- Keep ~/.dennou-aibou out of git; it contains credentials and session history.",
