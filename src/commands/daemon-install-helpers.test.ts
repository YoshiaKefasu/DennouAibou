...
function mockNodeGatewayPlanFixture(
  ...
  const {
    ...
    supported = true,
    warning,
    serviceEnvironment = { DENNOU_PORT: "3000" },
  } = params;
...
describe("buildGatewayInstallPlan", () => {
  // Prevent tests from reading the developer's real ~/.dennou-aibou/.env when
  ...
  it("uses provided nodePath and returns plan", async () => {
    ...
    expect(plan.environment).toEqual({ DENNOU_PORT: "3000" });
  ...
  it("merges config env vars into the environment", async () => {
    ...
    expect(plan.environment.DENNOU_PORT).toBe("3000");
...
describe("buildGatewayInstallPlan — dotenv merge", () => {
  ...
  it("merges .env file vars into the install plan", async () => {
    ...
    mockNodeGatewayPlanFixture({ serviceEnvironment: { DENNOU_PORT: "3000" } });
