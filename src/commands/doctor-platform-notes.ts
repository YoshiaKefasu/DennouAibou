...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const tokenEntries = [["DENNOU_GATEWAY_TOKEN", await getenv("DENNOU_GATEWAY_TOKEN")]] as const;
  const passwordEntries = [
    ["DENNOU_GATEWAY_PASSWORD", await getenv("DENNOU_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "DENNOU_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = env.DENNOU_NO_RESPAWN?.trim() ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- DENNOU_NO_RESPAWN is not set to 1; set it to avoid extra startup overhead from self-respawn.",
    );
  }
