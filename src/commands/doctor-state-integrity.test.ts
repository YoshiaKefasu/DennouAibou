...
type EnvSnapshot = {
  HOME?: string;
  DENNOU_HOME?: string;
  DENNOU_STATE_DIR?: string;
  DENNOU_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    DENNOU_HOME: process.env.DENNOU_HOME,
    DENNOU_STATE_DIR: process.env.DENNOU_STATE_DIR,
    DENNOU_OAUTH_DIR: process.env.DENNOU_OAUTH_DIR,
  };
}
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  it("prompts for oauth dir when DENNOU_OAUTH_DIR is explicitly configured", async () => {
    process.env.DENNOU_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: OpenClawConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(confirmRuntimeRepair).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
