...
function validateGatewayWebSocketUrl(value: string): string | undefined {
  ...
  if (
    !isSecureWebSocketUrl(trimmed, {
      allowPrivateWs: process.env.DENNOU_ALLOW_INSECURE_PRIVATE_WS === "1",
    })
  ) {
    return (
      "Use wss:// for remote hosts, or ws://127.0.0.1/localhost via SSH tunnel. " +
      "Break-glass: DENNOU_ALLOW_INSECURE_PRIVATE_WS=1 for trusted private networks."
    );
  }
  return undefined;
}
  ...
  if (authChoice === "token") {
    ...
    if (selectedMode === "ref") {
      const resolved = await promptSecretRefForSetup({
        ...
        config: cfg,
        prompter,
        preferredEnvVar: "DENNOU_GATEWAY_TOKEN",
        copy: {
          sourceMessage: "Where is this gateway token stored?",
          envVarPlaceholder: "DENNOU_GATEWAY_TOKEN",
        },
      });
      token = resolved.ref;
    } else {
    ...
    if (selectedMode === "ref") {
      const resolved = await promptSecretRefForSetup({
        ...
        config: cfg,
        prompter,
        preferredEnvVar: "DENNOU_GATEWAY_PASSWORD",
        copy: {
          sourceMessage: "Where is this gateway password stored?",
          envVarPlaceholder: "DENNOU_GATEWAY_PASSWORD",
        },
      });
