...
export async function buildGatewayInstallPlan(params: {
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      process.platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(params.env.DENNOU_PROFILE)
        : undefined,
  ...
  //   1. ~/.dennou-aibou/.env file vars  (lowest — user secrets / fallback keys)
  //   2. Config env vars              (dennou-aibou.json env.vars + inline keys)
  //   3. Auth-profile env refs        (credential store → env var lookups)
  //   4. Service environment          (HOME, PATH, DENNOU_* — highest)
  return {
    programArguments,
    workingDirectory,
