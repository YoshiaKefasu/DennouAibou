...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets DENNOU_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "openclaw",
    });

    expect(env.DENNOU_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("openclaw");
  });

  it("overrides pre-existing DENNOU_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      DENNOU_SHELL: "wrong",
    });
    expect(env.DENNOU_SHELL).toBe("acp-client");
  ...
  it("preserves DENNOU_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        DENNOU_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["DENNOU_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default OpenClaw bridge", () => {
    ...
    expect(env.DENNOU_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.DENNOU_SHELL).toBe("acp-client");
  });
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("DENNOU_API_KEY")).toBe(false);
  });
});
