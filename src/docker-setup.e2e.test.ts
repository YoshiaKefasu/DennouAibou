...
describe("scripts/docker/setup.sh", () => {
  ...
  it("reuses existing config token when DENNOU_GATEWAY_TOKEN is unset", async () => {
    ...
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      ...
      async (configDir) => {
        await writeFile(
          join(configDir, "dennou-aibou.json"),
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("DENNOU_GATEWAY_TOKEN=config-token-123"); // pragma: allowlist secret
  });
  ...
  it("reuses existing .env token when DENNOU_GATEWAY_TOKEN and config token are unset", async () => {
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("DENNOU_GATEWAY_TOKEN=dotenv-token-123"); // pragma: allowlist secret
  ...
  it("reuses the last non-empty .env token and strips CRLF without truncating '='", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        "DENNOU_GATEWAY_TOKEN=last=token=value\r", // pragma: allowlist secret
      ].join("\n"),
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("DENNOU_GATEWAY_TOKEN=last=token=value"); // pragma: allowlist secret
    expect(envFile).not.toContain("DENNOU_GATEWAY_TOKEN=first-token");
  ...
  it("rejects injected multiline DENNOU_EXTRA_MOUNTS values", async () => {
    ...
    expect(result.stderr).toContain("DENNOU_EXTRA_MOUNTS cannot contain control characters");
  });
  ...
  it("rejects invalid DENNOU_HOME_VOLUME names", async () => {
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("DENNOU_HOME_VOLUME must match");
  });
  ...
  it("rejects DENNOU_TZ values that are not present in zoneinfo", async () => {
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("DENNOU_TZ must match a timezone in /usr/share/zoneinfo");
  });
  ...
  it("keeps docker-compose gateway token env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/DENNOU_GATEWAY_TOKEN: \$\{DENNOU_GATEWAY_TOKEN:-\}/g)).toHaveLength(2);
  });

  it("keeps docker-compose timezone env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/TZ: \$\{DENNOU_TZ:-UTC\}/g)).toHaveLength(2);
  });
});
