...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.DENNOU_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.DENNOU_GATEWAY_PASSWORD);
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deepProbeResult?.authWarning) {
    findings.push({
      ...
      remediation: `Set DENNOU_GATEWAY_TOKEN/DENNOU_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("openclaw security audit --deep")}".`,
    });
  }
