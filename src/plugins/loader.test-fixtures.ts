...
let tempDirIndex = 0;
const prevBundledDir = process.env.DENNOU_BUNDLED_PLUGINS_DIR;
...
export function loadBundleFixture(params: {
  ...
  return withEnv({ DENNOU_STATE_DIR: stateDir, ...params.env }, () =>
    loadOpenClawPlugins({
      workspaceDir,
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.DENNOU_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
}
