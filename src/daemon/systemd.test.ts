...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    {
      name: "uses default service name when DENNOU_PROFILE is unset",
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/openclaw-gateway.service",
    },
    {
      name: "uses profile-specific service name when DENNOU_PROFILE is set to a custom value",
      env: { HOME: "/home/test", DENNOU_PROFILE: "jbphoenix" },
      expected: "/home/test/.config/systemd/user/openclaw-gateway-jbphoenix.service",
    },
    {
      name: "prefers DENNOU_SYSTEMD_UNIT over DENNOU_PROFILE",
      env: {
        HOME: "/home/test",
        DENNOU_PROFILE: "jbphoenix",
        DENNOU_SYSTEMD_UNIT: "custom-unit",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "handles DENNOU_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
        DENNOU_SYSTEMD_UNIT: "custom-unit.service",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "trims whitespace from DENNOU_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        DENNOU_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads DENNOU_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = mockReadGatewayServiceFile(
      ...
      { [`${TEST_SERVICE_HOME}/.openclaw/.env`]: "DENNOU_GATEWAY_TOKEN=env-file-token\n" },
    );

    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.DENNOU_GATEWAY_TOKEN).toBe("env-file-token");
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    mockReadGatewayServiceFile(
      [
        ...
        'Environment="DENNOU_GATEWAY_TOKEN=inline-token"',
      ],
      { [`${TEST_SERVICE_HOME}/.openclaw/.env`]: "DENNOU_GATEWAY_TOKEN=env-file-token\n" },
    );
    ...
    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.DENNOU_GATEWAY_TOKEN).toBe("env-file-token");
    expect(command?.environmentValueSources?.DENNOU_GATEWAY_TOKEN).toBe("file");
  });
  ...
  it("supports multiple EnvironmentFile entries and quoted paths", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue === "/home/test/.openclaw/first.env") {
        return "DENNOU_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.openclaw/second env.env") {
        return 'DENNOU_GATEWAY_PASSWORD="second password"\n'; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    expect(command?.environment).toEqual({
      ...
      DENNOU_GATEWAY_PASSWORD: "second password", // pragma: allowlist secret
    });
  });
  ...
  it("resolves relative EnvironmentFile paths from the unit directory", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/.config/systemd/user/gateway.env")) {
        return [
          "DENNOU_GATEWAY_TOKEN=relative-token", // pragma: allowlist secret
          "DENNOU_GATEWAY_PASSWORD=relative-password", // pragma: allowlist secret
        ].join("\n");
      }
      if (pathValue.endsWith("/.config/systemd/user/override.env")) {
        return "DENNOU_GATEWAY_TOKEN=override-token\n"; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      DENNOU_GATEWAY_TOKEN: "override-token",
      DENNOU_GATEWAY_PASSWORD: "relative-password", // pragma: allowlist secret
    });
  });
  ...
  it("parses EnvironmentFile content with comments and quoted values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue === "/home/test/.openclaw/gateway.env") {
        return [
          "# comment",
          "; another comment",
          'DENNOU_GATEWAY_TOKEN="quoted token"', // pragma: allowlist secret
          "DENNOU_GATEWAY_PASSWORD=quoted-password", // pragma: allowlist secret
        ].join("\n");
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      DENNOU_GATEWAY_TOKEN: "quoted token",
      DENNOU_GATEWAY_PASSWORD: "quoted-password", // pragma: allowlist secret
    });
    expect(command?.environmentValueSources).toEqual({
      DENNOU_GATEWAY_TOKEN: "file",
      DENNOU_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
    });
  });
});
...
describe("systemd service control", () => {
  ...
  it("restarts a profile-specific user unit", async () => {
    ...
    await assertRestartSuccess({ DENNOU_PROFILE: "work" });
  });
