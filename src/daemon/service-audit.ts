...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds DENNOU_GATEWAY_TOKEN and should be reinstalled.",
  ...
  issues.push({
    ...
    message:
      "Gateway service DENNOU_GATEWAY_TOKEN does not match gateway.auth.token in dennou-aibou.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.DENNOU_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.DENNOU_GATEWAY_TOKEN?.trim() || undefined;
}
