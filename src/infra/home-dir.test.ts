...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers DENNOU_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for DENNOU_HOME",
      env: { DENNOU_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when DENNOU_HOME is tilde-only and no fallback home exists",
      env: { DENNOU_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores DENNOU_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          DENNOU_HOME: "/srv/openclaw-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { DENNOU_HOME: "/srv/openclaw-home" } as NodeJS.ProcessEnv,
      },
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of DENNOU_HOME", () => {
    expect(
