...
function isDiscordExecApprovalClientEnabledForTest(params: {
  ...
  const accountConfig =
    accountId && accountId !== "default"
      ? (
          params.cfg.channels?.discordAccounts as
            | Record<string, { execApprovals?: { enabled?: boolean; approvers?: unknown[] } }>
            | undefined
        )?.[accountId]?.execApprovals
      : undefined;
...
function isTelegramExecApprovalClientEnabledForTest(params: {
  ...
  const accountConfig =
    accountId && accountId !== "default"
      ? (
          params.cfg.channels?.telegramAccounts as
            | Record<string, { execApprovals?: { enabled?: boolean; approvers?: unknown[] } }>
            | undefined
        )?.[accountId]?.execApprovals
      : undefined;
