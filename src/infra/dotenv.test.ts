...
describe("loadDotEnv", () => {
  it("loads ~/.dennou-aibou/.env as fallback without overriding CWD .env", async () => {
  ...
  it("loads the Ubuntu gateway.env compatibility fallback after ~/.dennou-aibou/.env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        process.env.HOME = base;
        const defaultStateDir = path.join(base, ".openclaw");
        process.env.DENNOU_STATE_DIR = defaultStateDir;
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "DENNOU_STATE_DIR=./evil-state",
            "DENNOU_CONFIG_PATH=./evil-config.json",
        ...
        expect(process.env.DENNOU_STATE_DIR).toBe(stateDir);
        expect(process.env.DENNOU_CONFIG_PATH).toBeUndefined();
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "DENNOU_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "DENNOU_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "DENNOU_LIVE_GEMINI_KEY=sk-gemini-live",
            "DENNOU_LIVE_OPENAI_KEY=sk-openai-live",
            "DENNOU_GATEWAY_TOKEN=attacker-token",
            "DENNOU_GATEWAY_PASSWORD=attacker-password",
            "DENNOU_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
        ...
        delete process.env.DENNOU_LIVE_ANTHROPIC_KEY;
        delete process.env.DENNOU_LIVE_ANTHROPIC_KEYS;
        delete process.env.DENNOU_LIVE_GEMINI_KEY;
        delete process.env.DENNOU_LIVE_OPENAI_KEY;
        delete process.env.DENNOU_GATEWAY_TOKEN;
        delete process.env.DENNOU_GATEWAY_PASSWORD;
        delete process.env.DENNOU_GATEWAY_SECRET;
        ...
        expect(process.env.DENNOU_LIVE_ANTHROPIC_KEY).toBeUndefined();
        expect(process.env.DENNOU_LIVE_ANTHROPIC_KEYS).toBeUndefined();
        expect(process.env.DENNOU_LIVE_GEMINI_KEY).toBeUndefined();
        expect(process.env.DENNOU_LIVE_OPENAI_KEY).toBeUndefined();
        expect(process.env.DENNOU_GATEWAY_TOKEN).toBeUndefined();
        expect(process.env.DENNOU_GATEWAY_PASSWORD).toBeUndefined();
        expect(process.env.DENNOU_GATEWAY_SECRET).toBeUndefined();
      });
    });
  });

  it("blocks DENNOU_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
        ...
        expect(process.env.DENNOU_STATE_DIR).toBeUndefined();
        expect(process.env.DENNOU_CONFIG_PATH).toBeUndefined();
      });
    });
  });

  it("blocks path-override vars (DENNOU_AGENT_DIR, DENNOU_BUNDLED_PLUGINS_DIR, PI_CODING_AGENT_DIR, DENNOU_OAUTH_DIR) from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "DENNOU_AGENT_DIR=./evil-agent",
            `DENNOU_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "PI_CODING_AGENT_DIR=./evil-coding",
            "DENNOU_OAUTH_DIR=./evil-oauth",
          ].join("\n"),
        );
        ...
        expect(process.env.DENNOU_AGENT_DIR).toBeUndefined();
        expect(process.env.DENNOU_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.PI_CODING_AGENT_DIR).toBeUndefined();
        expect(process.env.DENNOU_OAUTH_DIR).toBeUndefined();
      });
    });
  });

  it("blocks DENNOU_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "DENNOU_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.DENNOU_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.DENNOU_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "DENNOU_PINNED_PYTHON=./attacker-python",
            "DENNOU_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.DENNOU_PINNED_PYTHON;
        delete process.env.DENNOU_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.DENNOU_PINNED_PYTHON).toBeUndefined();
        expect(process.env.DENNOU_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "DENNOU_BUNDLED_HOOKS_DIR=./attacker-hooks",
            "DENNOU_BUNDLED_PLUGINS_DIR=./attacker-plugins",
            "DENNOU_BUNDLED_SKILLS_DIR=./attacker-skills",
          ].join("\n"),
        );

        delete process.env.DENNOU_BUNDLED_HOOKS_DIR;
        delete process.env.DENNOU_BUNDLED_PLUGINS_DIR;
        delete process.env.DENNOU_BUNDLED_SKILLS_DIR;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.DENNOU_BUNDLED_HOOKS_DIR).toBeUndefined();
        expect(process.env.DENNOU_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.DENNOU_BUNDLED_SKILLS_DIR).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "DENNOU_PINNED_PYTHON=/trusted/python",
            "DENNOU_PINNED_WRITE_PYTHON=/trusted/write-python",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        delete process.env.ANTHROPIC_BASE_URL;
        ...
        expect(process.env.DENNOU_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.DENNOU_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "DENNOU_LIVE_GEMINI_KEY=sk-gemini-live",
            "DENNOU_LIVE_OPENAI_KEY=sk-openai-live",
            "DENNOU_GATEWAY_TOKEN=trusted-token",
            "DENNOU_GATEWAY_PASSWORD=trusted-password",
            "DENNOU_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        delete process.env.ANTHROPIC_API_KEY;
        ...
        expect(process.env.DENNOU_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.DENNOU_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.DENNOU_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.DENNOU_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.DENNOU_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.DENNOU_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.DENNOU_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "DENNOU_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  ...
  it("does not load gateway.env when DENNOU_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        const customStateDir = path.join(base, "custom-state");
        process.env.HOME = base;
        process.env.DENNOU_STATE_DIR = customStateDir;
        await writeEnvFile(
          path.join(base, ".config", "openclaw", "gateway.env"),
          "FOO=from-gateway\n",
        );
        ...
        expect(process.env.DENNOU_STATE_DIR).toBe(customStateDir);
