...
const GATEWAY_DEVICE_ID_HEADER = "x-dennou-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-dennou-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-dennou-gateway-signed-at-ms";
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.DENNOU_APNS_RELAY_BASE_URL);
  ...
  const baseUrlSource = envBaseUrl
    ? "DENNOU_APNS_RELAY_BASE_URL"
    : "gateway.push.apns.relay.baseUrl";
  if (!baseUrl) {
    return {
      ok: false,
      error:
        "APNs relay config missing: set gateway.push.apns.relay.baseUrl or DENNOU_APNS_RELAY_BASE_URL",
    };
  }
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.DENNOU_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error(
        "http relay URLs require DENNOU_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
    ...
    return {
      ...
      value: {
        ...
        timeoutMs: normalizeTimeoutMs(
          env.DENNOU_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs,
        ),
      },
    };
  } catch (err) {
