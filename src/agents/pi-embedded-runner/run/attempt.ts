...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const toolsRaw = params.disableTools
      ...
      : (() => {
          const allTools = createOpenClawCodingTools({
            ...
            // TODO(debloat): pi-ai compat carries upstream thinking formats;
            // the debloated config surface only accepts "openai". Same cast as
            // extractModelCompat; remove once pi-ai drops "openrouter"/"zai".
    ...
    try {
      ...
      // DefaultResourceLoader.reload() rehydrates settings from disk and can drop
      // OpenClaw compaction overrides applied earlier.
      applyPiCompactionSettingsFromConfig({
        settingsManager,
        cfg: params.config,
        contextTokenBudget: params.contextTokenBudget,
      ...
      try {
        ...
        const googlePromptCacheStreamFn = await prepareGooglePromptCacheStreamFn({
          ...
          sessionManager,
          signal: runAbortController.signal,
          streamFn: activeSession.agent.streamFunction,
          systemPrompt: systemPromptText,
        });
