...
import {
  ...
  DEFAULT_DENNOU_BROWSER_COLOR,
  DEFAULT_DENNOU_BROWSER_PROFILE_NAME,
  resolveProfile,
...
const CDP_SOURCE_RANGE_ENV_KEY = "DENNOU_BROWSER_CDP_SOURCE_RANGE";
...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_DENNOU_BROWSER_COLOR,
    executablePath: undefined,
    headless: params.headless,
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_DENNOU_BROWSER_PROFILE_NAME,
    extraArgs: [],
    profiles: {
      [DEFAULT_DENNOU_BROWSER_PROFILE_NAME]: {
...
export async function ensureSandboxBrowser(params: {
  ...
  if (!hasContainer) {
    ...
    args.push("-e", `DENNOU_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `DENNOU_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `DENNOU_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    if (cdpSourceRange) {
      args.push("-e", `${CDP_SOURCE_RANGE_ENV_KEY}=${cdpSourceRange}`);
    }
    args.push("-e", `DENNOU_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `DENNOU_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
    ...
    args.push("-e", "DENNOU_BROWSER_NO_SANDBOX=1");
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_DENNOU_BROWSER_PROFILE_NAME)
    : null;
