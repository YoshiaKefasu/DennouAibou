...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["DENNOU_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns DENNOU_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-bundled-override-"));
    process.env.DENNOU_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
