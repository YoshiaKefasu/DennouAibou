...
const ETC_DENNOU_DIR = path.join(ROOT_DIR, "etc", "openclaw");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "dennou-aibou.json");
...
function etcOpenClawPath(...parts: string[]) {
  return path.join(ETC_DENNOU_DIR, ...parts);
}
    ...
    expectResolveIncludeError(
      () =>
        resolve(
          ...
          configPath("sub", "dennou-aibou.json"),
        ),
  ...
  describe("edge cases", () => {
    ...
    it("allows include files when the config root path is a symlink", async () => {
      ...
      try {
        ...
        const result = resolveConfigIncludes(
          ...
          path.join(linkRoot, "dennou-aibou.json"),
        );
    ...
    it("rejects oversized include files", async () => {
      ...
      try {
        ...
        expect(() =>
          resolveConfigIncludes({ $include: "./big.json5" }, path.join(configDir, "dennou-aibou.json")),
