...
type SaveSessionStoreOptions = {
   ...
   * When omitted, saveSessionStoreUnlocked resolves it via loadConfig().
   */
...
async function saveSessionStoreUnlocked(
  ...
  if (!opts?.skipMaintenance) {
    ...
    // Resolve protected-session config once for maintenance exclusion.
    const protectedSessionCfg: ProtectedSessionConfig | undefined =
      opts?.protectedSessionCfg ?? loadConfig();
...
function getOrCreateLockQueue(storePath: string): SessionStoreLockQueue {
  ...
  const created: SessionStoreLockQueue = {
    running: false,
    pending: [],
    drainPromise: null,
  };
