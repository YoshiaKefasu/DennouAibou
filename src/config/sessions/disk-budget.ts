...
export async function enforceSessionDiskBudget(params: {
  ...
  if (total > highWaterBytes) {
    ...
    for (const key of keys) {
      ...
      if (
        // Belt-and-suspenders: isProtectedSessionKey handles normalized config; raw activeSessionKey is fallback when cfg is absent.
        isProtectedSessionKey(key, params.protectedSessionCfg) ||
        (activeSessionKey && key.trim().toLowerCase() === activeSessionKey)
      ) {
        continue;
      }
