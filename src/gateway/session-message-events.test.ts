...
describe("session.message websocket events", () => {
  test("includes spawned session ownership metadata on lifecycle sessions.changed events", async () => {
    const previousMinimalGateway = process.env.DENNOU_TEST_MINIMAL_GATEWAY;
    delete process.env.DENNOU_TEST_MINIMAL_GATEWAY;
    try {
      ...
      if (previousMinimalGateway === undefined) {
        ...
        process.env.DENNOU_TEST_MINIMAL_GATEWAY = previousMinimalGateway;
      }
    }
  });
