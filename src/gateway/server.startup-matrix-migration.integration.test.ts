...
describe("gateway startup channel maintenance wiring", () => {
  it("runs startup channel maintenance with the resolved startup config", async () => {
    const previousBundledPluginsDir = process.env.DENNOU_BUNDLED_PLUGINS_DIR;
    const previousSkipChannels = process.env.DENNOU_SKIP_CHANNELS;
    process.env.DENNOU_BUNDLED_PLUGINS_DIR = path.resolve(process.cwd(), "extensions");
    ...
    try {
      ...
      if (previousBundledPluginsDir === undefined) {
        ...
        process.env.DENNOU_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
      }
      if (previousSkipChannels === undefined) {
        delete process.env.DENNOU_SKIP_CHANNELS;
      } else {
        process.env.DENNOU_SKIP_CHANNELS = previousSkipChannels;
      }
      clearPluginDiscoveryCache();
      clearPluginManifestRegistryCache();
    }
