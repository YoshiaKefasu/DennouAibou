...
export type AgentHandlersDeps = {
  listAgentIds?: typeof listAgentIds;
  loadConfig?: typeof loadConfig;
  loadSessionEntry?: typeof loadSessionEntry;
  loadGatewaySessionRow?: typeof loadGatewaySessionRow;
  updateSessionStore?: typeof updateSessionStore;
  resolveAgentIdFromSessionKey?: typeof resolveAgentIdFromSessionKey;
  resolveExplicitAgentSessionKey?: typeof resolveExplicitAgentSessionKey;
  resolveAgentMainSessionKey?: typeof resolveAgentMainSessionKey;
  agentCommandFromIngress?: typeof agentCommandFromIngress;
  registerAgentRunContext?: typeof registerAgentRunContext;
  reactivateCompletedSubagentSession?: typeof reactivateCompletedSubagentSession;
  performGatewaySessionReset?: typeof performGatewaySessionReset;
  resolveSendPolicy?: typeof resolveSendPolicy;
  normalizeSessionDeliveryFields?: typeof normalizeSessionDeliveryFields;
};

const defaultAgentHandlersDeps: Required<AgentHandlersDeps> = {
  listAgentIds,
  loadConfig,
  loadSessionEntry,
  loadGatewaySessionRow,
...
async function runSessionResetFromAgent(
  params: {
    key: string;
    reason: "new" | "reset";
  },
  deps: Required<AgentHandlersDeps>,
): Promise<
  | { ok: true; key: string; sessionId?: string }
  | { ok: false; error: ReturnType<typeof errorShape> }
> {
  const result = await deps.performGatewaySessionReset({
    key: params.key,
...
function emitSessionsChanged(
  ...
  const sessionRow = payload.sessionKey ? deps.loadGatewaySessionRow(payload.sessionKey) : null;
...
export function createAgentHandlers(overrides: AgentHandlersDeps = {}): GatewayRequestHandlers {
  const deps = { ...defaultAgentHandlersDeps, ...overrides };
  const agentHandlers: GatewayRequestHandlers = {
    agent: async ({ params, respond, context, client, isWebchatConnect }) => {
      ...
      if (agentId) {
        const knownAgents = deps.listAgentIds(cfg);
      ...
      let requestedSessionKey =
        requestedSessionKeyRaw ??
        deps.resolveExplicitAgentSessionKey({
          cfg,
          agentId,
        });
      if (agentId && requestedSessionKeyRaw) {
        const sessionAgentId = deps.resolveAgentIdFromSessionKey(requestedSessionKeyRaw);
      ...
      if (requestedSessionKey) {
        const { cfg, storePath, entry, canonicalKey } = deps.loadSessionEntry(requestedSessionKey);
        cfgForAgent = cfg;
        isNewSession = !entry;
        const now = Date.now();
        const sessionId = entry?.sessionId ?? randomUUID();
        const labelValue = request.label?.trim() || entry?.label;
        const sessionAgent = deps.resolveAgentIdFromSessionKey(canonicalKey);
        ...
        if (spawnedByValue && (!resolvedGroupId || !resolvedGroupChannel || !resolvedGroupSpace)) {
          try {
            const parentEntry = deps.loadSessionEntry(spawnedByValue)?.entry;
            inheritedGroup = {
        ...
        const deliveryFields = deps.normalizeSessionDeliveryFields(entry);
        ...
        const sendPolicy = deps.resolveSendPolicy({
          cfg,
          entry,
        ...
        const agentId = deps.resolveAgentIdFromSessionKey(canonicalSessionKey);
        const mainSessionKey = deps.resolveAgentMainSessionKey({ cfg, agentId });
        if (storePath) {
          const persisted = await deps.updateSessionStore(storePath, (store) => {
        ...
        deps.registerAgentRunContext(idem, { sessionKey: canonicalSessionKey });
      }
      ...
      if (resolvedSessionKey) {
        await deps.reactivateCompletedSubagentSession({
    ...
    "agent.identity.get": ({ params, respond }) => {
      ...
      if (sessionKeyRaw) {
        ...
        const resolved = deps.resolveAgentIdFromSessionKey(sessionKeyRaw);
