...
const connectNodeClient = async (params: {
  ...
  const token = process.env.DENNOU_GATEWAY_TOKEN;
  if (!token) {
    throw new Error("DENNOU_GATEWAY_TOKEN is required for node test clients");
  }
...
function getGatewayTestConfigPath(): string {
  ...
  if (!configPath) {
    throw new Error("DENNOU_CONFIG_PATH is required in the gateway test environment");
  }
  return configPath;
}
