...
describe("callGateway url resolution", () => {
  const envSnapshot = captureEnv([
    "DENNOU_ALLOW_INSECURE_PRIVATE_WS",
    "DENNOU_GATEWAY_URL",
  ...
  beforeEach(() => {
    envSnapshot.restore();
    delete process.env.DENNOU_ALLOW_INSECURE_PRIVATE_WS;
  ...
  it("uses DENNOU_GATEWAY_URL env override in remote mode when remote URL is missing", async () => {
    loadConfig.mockReturnValue({
      gateway: { mode: "remote", bind: "loopback", remote: {} },
    });
    resolveGatewayPort.mockReturnValue(18789);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    process.env.DENNOU_GATEWAY_URL = "wss://gateway-in-container.internal:9443/ws";
    process.env.DENNOU_GATEWAY_TOKEN = "env-token";
...
describe("buildGatewayConnectionDetails", () => {
  ...
  it("uses env DENNOU_GATEWAY_URL when set", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local", bind: "loopback" } });
    resolveGatewayPort.mockReturnValue(18800);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    const prevUrl = process.env.DENNOU_GATEWAY_URL;
    try {
      process.env.DENNOU_GATEWAY_URL = "wss://browser-gateway.local:9443/ws";

      const details = buildGatewayConnectionDetails();

      expect(details.url).toBe("wss://browser-gateway.local:9443/ws");
      expect(details.urlSource).toBe("env DENNOU_GATEWAY_URL");
      expect(details.bindDetail).toBeUndefined();
    } finally {
      if (prevUrl === undefined) {
        delete process.env.DENNOU_GATEWAY_URL;
      } else {
        process.env.DENNOU_GATEWAY_URL = prevUrl;
      }
    }
  });
  ...
  it("allows ws:// private remote URLs only when DENNOU_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.DENNOU_ALLOW_INSECURE_PRIVATE_WS = "1";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
        bind: "loopback",
  ...
  it("allows ws:// hostname remote URLs when DENNOU_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.DENNOU_ALLOW_INSECURE_PRIVATE_WS = "1";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
...
describe("callGateway url override auth requirements", () => {
  ...
  it("throws when url override is set without explicit credentials", async () => {
    process.env.DENNOU_GATEWAY_TOKEN = "env-token";
    process.env.DENNOU_GATEWAY_PASSWORD = "env-password";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
  ...
  it("throws when env URL override is set without env credentials", async () => {
    process.env.DENNOU_GATEWAY_URL = "wss://override.example/ws";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
        auth: { token: "local-token", password: "local-password" },
...
describe("callGateway password resolution", () => {
  ...
  const explicitAuthCases = [
    {
      ...
      envKey: "DENNOU_GATEWAY_PASSWORD",
      envValue: "from-env",
  ...
  it.each([
    ...
    if (envPassword !== undefined) {
      process.env.DENNOU_GATEWAY_PASSWORD = envPassword;
    }
  ...
  it("does not resolve local password ref when env password takes precedence", async () => {
    process.env.DENNOU_GATEWAY_PASSWORD = "from-env";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
        bind: "loopback",
