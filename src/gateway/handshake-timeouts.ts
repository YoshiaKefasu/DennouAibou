export const DEFAULT_PREAUTH_HANDSHAKE_TIMEOUT_MS = 12_000;
...
export function getConnectChallengeTimeoutMsFromEnv(
  ...
  const raw = env.DENNOU_CONNECT_CHALLENGE_TIMEOUT_MS;
  if (raw) {
...
export function getPreauthHandshakeTimeoutMsFromEnv(env: NodeJS.ProcessEnv = process.env): number {
  const configuredTimeout =
    env.DENNOU_HANDSHAKE_TIMEOUT_MS || (env.VITEST && env.DENNOU_TEST_HANDSHAKE_TIMEOUT_MS);
