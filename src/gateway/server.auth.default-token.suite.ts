...
export function registerDefaultAuthTokenSuite(): void {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.DENNOU_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.DENNOU_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        const ws = await openWs(port);
        ...
        if (prevHandshakeTimeout === undefined) {
          ...
          process.env.DENNOU_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });

    test("prefers DENNOU_HANDSHAKE_TIMEOUT_MS and falls back on empty string", () => {
      const prevHandshakeTimeout = process.env.DENNOU_HANDSHAKE_TIMEOUT_MS;
      const prevTestHandshakeTimeout = process.env.DENNOU_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.DENNOU_HANDSHAKE_TIMEOUT_MS = "75";
      process.env.DENNOU_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        expect(getPreauthHandshakeTimeoutMsFromEnv()).toBe(75);
        process.env.DENNOU_HANDSHAKE_TIMEOUT_MS = "";
        expect(getPreauthHandshakeTimeoutMsFromEnv()).toBe(20);
      } finally {
        if (prevHandshakeTimeout === undefined) {
          delete process.env.DENNOU_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.DENNOU_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
        if (prevTestHandshakeTimeout === undefined) {
          delete process.env.DENNOU_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.DENNOU_TEST_HANDSHAKE_TIMEOUT_MS = prevTestHandshakeTimeout;
        }
      }
    });
      ...
      for (const testCase of [
        {
          env: {
            DENNOU_VERSION: " ",
            DENNOU_SERVICE_VERSION: "2.4.6-service",
