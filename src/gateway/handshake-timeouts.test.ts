...
describe("gateway handshake timeouts", () => {
  ...
  test("prefers DENNOU_HANDSHAKE_TIMEOUT_MS and falls back on the test-only env", () => {
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        DENNOU_HANDSHAKE_TIMEOUT_MS: "75",
        DENNOU_TEST_HANDSHAKE_TIMEOUT_MS: "20",
      }),
    ).toBe(75);
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        DENNOU_HANDSHAKE_TIMEOUT_MS: "",
        DENNOU_TEST_HANDSHAKE_TIMEOUT_MS: "20",
        VITEST: "1",
      }),
    ).toBe(20);
  ...
  test("getConnectChallengeTimeoutMsFromEnv reads DENNOU_CONNECT_CHALLENGE_TIMEOUT_MS", () => {
    expect(getConnectChallengeTimeoutMsFromEnv({})).toBeUndefined();
    expect(
      getConnectChallengeTimeoutMsFromEnv({ DENNOU_CONNECT_CHALLENGE_TIMEOUT_MS: "15000" }),
    ).toBe(15_000);
    expect(
      getConnectChallengeTimeoutMsFromEnv({ DENNOU_CONNECT_CHALLENGE_TIMEOUT_MS: "garbage" }),
    ).toBeUndefined();
  });

  test("resolveConnectChallengeTimeoutMs falls back to env override", () => {
    const original = process.env.DENNOU_CONNECT_CHALLENGE_TIMEOUT_MS;
    try {
      process.env.DENNOU_CONNECT_CHALLENGE_TIMEOUT_MS = "5000";
      ...
      if (original === undefined) {
        delete process.env.DENNOU_CONNECT_CHALLENGE_TIMEOUT_MS;
      } else {
        process.env.DENNOU_CONNECT_CHALLENGE_TIMEOUT_MS = original;
      }
    }
  });
});
