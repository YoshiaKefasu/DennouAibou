...
const ACP_BIND_LIVE = isTruthyEnvValue(process.env.DENNOU_LIVE_ACP_BIND);
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        port: process.env.DENNOU_GATEWAY_PORT,
      ...
      const liveAgent = normalizeAcpAgent(process.env.DENNOU_LIVE_ACP_BIND_AGENT);
      const agentCommandOverride =
        process.env.DENNOU_LIVE_ACP_BIND_AGENT_COMMAND?.trim() || undefined;
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-live-acp-bind-"));
      const tempStateDir = path.join(tempRoot, "state");
      const tempConfigPath = path.join(tempRoot, "dennou-aibou.json");
      ...
      try {
        ...
        if (previous.port === undefined) {
          ...
          process.env.DENNOU_GATEWAY_PORT = previous.port;
        }
