...
const UNKNOWN_CLIENT_IP_BUDGET_KEY = "__DENNOU_unknown_client_ip__";

export function getMaxPreauthConnectionsPerIpFromEnv(env: NodeJS.ProcessEnv = process.env): number {
  const configured =
    env.DENNOU_MAX_PREAUTH_CONNECTIONS_PER_IP ||
    (env.VITEST && env.DENNOU_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP);
  if (!configured) {
