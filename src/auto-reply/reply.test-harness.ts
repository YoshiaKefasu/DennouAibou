...
type HomeEnvSnapshot = {
  ...
  DENNOU_STATE_DIR: string | undefined;
  DENNOU_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    DENNOU_AGENT_DIR: process.env.DENNOU_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    process.env.DENNOU_STATE_DIR = path.join(home, ".openclaw");
    process.env.DENNOU_AGENT_DIR = path.join(home, ".openclaw", "agent");
