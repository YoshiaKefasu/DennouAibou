...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    openclawHome: process.env.DENNOU_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("DENNOU_HOME", snapshot.openclawHome);
  restoreKey("DENNOU_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.DENNOU_STATE_DIR = join(home, ".openclaw");
