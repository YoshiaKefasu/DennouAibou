...
- この 250K 領域に対しては、要約による圧縮や会話メッセージの改変を一切行わず、1文字も削らない不可侵領域とする。
  （※なお、ツール実行結果のカンナくず処理は §4 の通常ルールに従い、4ターン以前のものはプレースホルダー化される。7.3 が保護するのは「会話メッセージ本体 (User/Assistant) および直近3ターンの文脈」である）。
- 要約の対象となるのは、常に「直近 250K より古い過去ブロック」のみとする。

### 7.4 フェイルセーフ：一時ドロップ（Temporary Eviction）安全弁
