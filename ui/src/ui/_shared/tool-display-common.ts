/**
 * UI-local mirror of `src/agents/tool-display-common.ts`.
 *
 * Source of truth: `src/agents/tool-display-common.ts` (HEAD).
 *
 * The upstream file is pure (no Node-only globals), but importing it pulls
 * the entire `src/` Node layer into the control UI bundle and re-introduces
 * the `process is not defined` crash this commit series is fixing.
 */
import { resolveExecDetail } from "./tool-display-exec.ts";
