/**
 * UI-local mirror of `src/shared/chat-message-content.ts`.
 *
 * Source of truth: `src/shared/chat-message-content.ts` (HEAD).
 *
 * The UI uses these helpers to render assistant text blocks honouring the
 * commentary / final_answer phase split. Pure (no Node-only globals) — we
 * mirror locally only to avoid pulling the entire `src/` Node layer into
 * the browser bundle.
 */
