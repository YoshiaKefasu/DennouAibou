/**
 * UI-local mirror of `src/chat/tool-content.ts`.
 *
 * Pure type/function set (no Node-only globals). Mirrored here so the UI
 * does not import from `src/`, which would pull the entire Node layer into
 * the browser bundle and re-introduce the `process is not defined` crash.
 *
 * Source of truth: `src/chat/tool-content.ts` (HEAD).
 */
