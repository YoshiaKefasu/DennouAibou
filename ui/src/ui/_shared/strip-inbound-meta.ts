/**
 * UI-local mirror of `src/auto-reply/reply/strip-inbound-meta.ts`.
 *
 * The upstream implementation depends on `zod` and `safeParseJsonWithSchema`
 * from `src/utils/zod-parse.ts`, neither of which is safe to evaluate in a
 * browser bundle. Importing the upstream file pulls the entire `src/` Node
 * layer (and its transitive `process.env` / `node:os` references) into the
 * control UI bundle, re-introducing the `process is not defined` crash this
 * commit series is fixing.
 *
 * The UI uses these functions purely for *display sanitisation* (hiding
 * AI-facing metadata blocks from user-visible chat history). It never feeds
 * the parsed blocks into a code path that needs strict zod validation, so
 * we replace the zod schema with a permissive `JSON.parse` wrapped in
 * try/catch. Behaviour of the three exported functions is kept identical
 * to upstream on the success / failure paths the UI exercises.
 *
 * Source of truth: `src/auto-reply/reply/strip-inbound-meta.ts` (HEAD).
 */
...
function parseInboundMetaBlock(lines: string[], sentinel: string): Record<string, unknown> | null {
  for (let i = 0; i < lines.length; i++) {
    ...
    // UI-local: no zod, just JSON.parse. The UI uses the parsed object only
    // for display hints (sender label), not for any security-sensitive path,
    // so a permissive parse is acceptable here.
    try {
 ...
 * present (fast path — zero allocation).
 */
