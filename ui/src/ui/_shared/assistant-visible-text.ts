/**
 * UI-local mirror of `src/shared/text/assistant-visible-text.ts`.
 *
 * The upstream file depends on a small graph of pure text helpers
 * (`code-regions`, `reasoning-tags`, `model-special-tokens`) that is
 * otherwise fine to import in isolation. We mirror the whole graph here
 * so the UI bundle does not need to import from `src/`, which would
 * pull the entire Node layer (process.env / node:os) into the browser
 * bundle and re-introduce the `process is not defined` crash this
 * commit series is fixing.
 *
 * Source of truth: `src/shared/text/assistant-visible-text.ts` (HEAD).
 * When upstream semantics change, edit both files together.
 */
import { findCodeRegions, isInsideCode } from "./code-regions.ts";
import { stripModelSpecialTokens } from "./model-special-tokens.ts";
