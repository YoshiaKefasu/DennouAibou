...
- **Nuclear Method:** If an upstream plugin is complete garbage and we will never use it, delete the _entire_ plugin folder. Git handles folder-level deletions/additions flawlessly during merges.

## Rule 3: Defend the Hooks

**If the upstream breaks the very hooks we rely on, we fix their code.**
As seen with the `cli-runner` regression, sometimes the upstream architects make mistakes that silently disable critical expansion points (`before_prompt_build`, etc.). In these cases, we proactively patch the core files to restore architectural sanity. We do this cleanly, mimicking how they _should_ have done it, so that when they finally issue an official fix, our code merges with theirs seamlessly.
...
**Use `--changed` only for test runs; never run the full `pnpm test` in agent loops.**

The repo's `pnpm test` wrapper (`scripts/test-projects.mjs`) expands to the whole matching project group and saturates CPU (100%) for minutes — it must NOT be used by agents (code-reviewer / executor) during daily loops. Measured on this fork:

- `pnpm test:changed` — runs ONLY tests for files changed vs `origin/main` (fast, the standard agent test command)
...
- `pnpm test` — **BANNED in agent loops** (CPU 100%, minutes); full-suite runs require explicit user approval

Standard usage:

```bash
# standard agent test command — changed-only
pnpm test:changed

# targeted file (daily loop, when a specific suite is affected)
bunx vitest run src/dennou-soul/prune-engine.test.ts

# full suite / landing gate — REQUIRES EXPLICIT USER APPROVAL
# pnpm test  ← do not run without approval
...
Do NOT use `bun test` directly for vitest-based suites (vi.hoisted crashes). Do NOT use `bunx vitest run --project <config>` without confirming the exact project name (it errors on this fork's config names). When instructing executor/code-reviewer agents, always pass explicit commands: `pnpm test:changed` for broad verification, or `bunx vitest run <file>` for a specific suite. Never leave test-command choice to the agent.

---

_Follow these rules, and DennouAibou will outlive the tools it was born from._
