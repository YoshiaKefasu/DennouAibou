...
DennouAibou gives agents tools to work across sessions, inspect status, and
...
After the target responds, DennouAibou can run a **reply-back loop** where the
...
`subagents` is the control-plane helper for already spawned DennouAibou
...
the completion origin only identifies a channel DennouAibou can still reuse the
