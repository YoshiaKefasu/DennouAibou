...
alongside DennouAibou. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, DennouAibou falls back to the
...
DennouAibou creates a self-contained QMD home under
...
- DennouAibou creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). If a mode fails, DennouAibou retries with `qmd query`.
- If QMD fails entirely, DennouAibou falls back to the builtin SQLite engine.
...
process, so you can tune QMD globally without adding new DennouAibou config:
...
When scope denies a search, DennouAibou logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If DennouAibou
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs DennouAibou uses.
...
DennouAibou's builtin symlink rules. Keep temporary monorepo checkouts under
