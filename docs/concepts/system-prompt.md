---
summary: "What the DennouAibou system prompt contains and how it is assembled"
read_when:
...
DennouAibou builds a custom system prompt for every agent run. The prompt is **DennouAibou-owned** and does not use the pi-coding-agent default prompt.

The prompt is assembled by DennouAibou and injected into each agent run.

Provider plugins can contribute cache-aware prompt guidance without replacing
the full DennouAibou-owned prompt. The provider runtime can:
...
- **DennouAibou Self-Update**: how to inspect config safely with
...
DennouAibou can render smaller system prompts for sub-agents. The runtime sets a
...
occurs, DennouAibou can inject a warning block in Project Context; control this with
...
When eligible skills exist, DennouAibou injects a compact **available skills list**
...
local DennouAibou docs directory (either `docs/` in the repo workspace or the bundled npm
...
for DennouAibou behavior, commands, configuration, or architecture, and to run
