---
summary: "How DennouAibou summarizes long conversations to stay within model limits"
read_when:
...
When a conversation approaches that limit, DennouAibou **compacts** older messages
...
When DennouAibou splits history into compaction chunks, it keeps assistant tool
calls paired with their matching `toolResult` entries. If a split point lands
inside a tool block, DennouAibou moves the boundary so the pair stays together and
...
DennouAibou compacts and retries). Typical overflow signatures include
...
<Info>
Before compacting, DennouAibou automatically reminds the agent to save important
