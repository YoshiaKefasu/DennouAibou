...
read_when:
  - You want to understand how DennouAibou assembles model context
...
A **context engine** controls how DennouAibou builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

DennouAibou ships with a built-in `legacy` engine. Plugins can register
...
Context engine plugins are installed like any other DennouAibou plugin. Install
...
Every time DennouAibou runs a model prompt, the context engine participates at
...
DennouAibou currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. DennouAibou
...
The built-in `legacy` engine preserves DennouAibou's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. DennouAibou uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. DennouAibou disables Pi's built-in
...
`ownsCompaction: false` does **not** mean DennouAibou automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  ...
  DennouAibou's built-in compaction behavior.
...
DennouAibou resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to DennouAibou's built-in summarization. Plugin engines can implement
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, DennouAibou
