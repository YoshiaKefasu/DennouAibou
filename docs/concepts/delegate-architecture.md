---
summary: "Delegate architecture: running DennouAibou as a named agent on behalf of an organization"
...
Goal: run DennouAibou as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an DennouAibou agent that:
...
DennouAibou's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of DennouAibou's own tool policy.
...
All delegate actions flow through DennouAibou's session store. For compliance, ensure these logs are retained and reviewed.
...
recall view. DennouAibou redacts credential/token-like text, truncates long
