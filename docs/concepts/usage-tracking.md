...
- `/usage cost` in chats: local cost summary aggregated from DennouAibou session logs.
...
- **MiniMax**: API key or MiniMax OAuth auth profile. DennouAibou treats
  ...
  quota, so DennouAibou inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, DennouAibou prefers the
...
can supply plugin-specific usage auth logic; otherwise DennouAibou falls back to
