...
read_when:
  - Building or debugging native DennouAibou plugins
...
This page covers the internal architecture of the DennouAibou plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside DennouAibou. Every
native DennouAibou plugin registers against one or more capability types:
...
DennouAibou classifies every loaded plugin into a shape based on its actual
...
DennouAibou's plugin system has four layers:

1. **Manifest + discovery**
   DennouAibou finds candidate plugins from configured paths, workspace roots,
...
3. **Runtime loading**
   Native DennouAibou plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of DennouAibou reads the registry to expose tools, channels, provider
...
That keeps plugin-owned CLI code inside the plugin while still letting DennouAibou
...
That split lets DennouAibou validate config, explain missing/disabled plugins, and
...
normal chat actions. DennouAibou keeps one shared `message` tool in core, and
...
DennouAibou treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's DennouAibou-facing
  surfaces
...
So if DennouAibou adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If DennouAibou has shared
...
```ts
import type { DennouAibouPluginDefinition } from "openclaw/plugin-sdk/plugin-entry";
import {
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "openclaw/plugin-sdk/media-understanding";

const plugin: DennouAibouPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
DennouAibou already treats image/audio/video understanding as one shared
...
`DennouAibouPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   DennouAibou can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that DennouAibou knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `DennouAibouPluginApi` or
  `api.runtime`
...
- a malicious native plugin is equivalent to arbitrary code execution inside
  the DennouAibou process

Compatible bundles are safer by default because DennouAibou currently treats them
...
DennouAibou exports capabilities, not implementation convenience.
...
At startup, DennouAibou does roughly this:
...
The manifest is the control-plane source of truth. DennouAibou uses it to:
...
DennouAibou keeps short in-process caches for:
...
DennouAibou still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, DennouAibou calls hooks in this rough order.
...
| --  | _(built-in model lookup)_         | DennouAibou tries the normal registry/catalog path first                                 | _(not a plugin hook)_                                                                                                                       |
...
that still runs on DennouAibou's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before DennouAibou's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as DennouAibou adds those
...
- DennouAibou only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "DennouAibou plugin runtime helpers",
    count: 5,
  },
});
```
...
- Bundled extension internals remain private. External plugins should use only
  `openclaw/plugin-sdk/*` subpaths. DennouAibou core/test code may use the repo
...
- Facade-loaded public entry points prefer the active runtime config snapshot
  ...
  DennouAibou is not yet serving a runtime snapshot.
...
`catalog.run(...)` returns the same shape DennouAibou writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to DennouAibou's
...
- if both `catalog` and `discovery` are registered, DennouAibou uses `catalog`
...
When DennouAibou needs setup surfaces for a disabled channel plugin, or
...
this flag. Keep the plugin on the default behavior and let DennouAibou load the
...
DennouAibou can also merge **external channel catalogs** (for example, an MPM
...
2. add typed plugin registration/runtime surfaces
   Extend `DennouAibouPluginApi` and/or `api.runtime` with the smallest useful
...
This is how DennouAibou stays opinionated without becoming hardcoded to one
