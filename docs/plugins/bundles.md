---
summary: "Install and use Codex, Claude, and Cursor bundles as DennouAibou plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how DennouAibou maps bundle content into native features
...
DennouAibou can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
DennouAibou maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native DennouAibou plugins. Native plugins run
...
of requiring authors to rewrite them as native DennouAibou plugins, DennouAibou
...
## What DennouAibou maps from bundles

Not every bundle feature runs in DennouAibou today. Here is what works and what
...
- bundle skill roots load as normal DennouAibou skill roots
- Claude `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means Claude markdown command files work through the normal DennouAibou skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal DennouAibou hook-pack
...
- DennouAibou merges bundle MCP config into the effective embedded Pi settings as
  `mcpServers`
- DennouAibou exposes supported bundle MCP tools during embedded Pi agent turns by
...
- `transport` may be set to `"streamable-http"` or `"sse"`; when omitted, DennouAibou uses `sse`
...
DennouAibou registers bundle MCP tools with provider-safe names in the form
...
- Claude `settings.json` is imported as default embedded Pi settings when the
  bundle is enabled
- DennouAibou sanitizes shell override keys before applying them
...
- DennouAibou loads `.lsp.json` plus any manifest-declared `lspServers` paths
...
These are recognized and shown in diagnostics, but DennouAibou does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit DennouAibou best when they use skill roots and DennouAibou-style
...
DennouAibou checks for native plugin format first:
...
If a directory contains both, DennouAibou uses the native path. This prevents
...
- DennouAibou does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. DennouAibou does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    DennouAibou hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
