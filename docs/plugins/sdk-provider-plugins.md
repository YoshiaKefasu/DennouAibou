...
summary: "Step-by-step guide to building a model provider plugin for DennouAibou"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to DennouAibou
...
(LLM) to DennouAibou. By the end you will have a provider with a model catalog,
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    The manifest declares `providerAuthEnvVars` so DennouAibou can detect
    credentials without loading your plugin runtime. `modelSupport` is optional
    and lets DennouAibou auto-load your provider plugin from shorthand model ids
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    <Accordion title="All available provider hooks">
      DennouAibou calls hooks in this order. Most providers only use 2-3:
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    DennouAibou classifies this as a **hybrid-capability** plugin. This is the
