...
Bind a Task Flow runtime to an existing DennouAibou session key or trusted tool
...
trusted DennouAibou session key from your own binding layer. Do not bind from raw
user input.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: { query: "DennouAibou plugin SDK", count: 5 },
});
```
