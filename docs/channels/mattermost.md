---
summary: "Mattermost bot setup and DennouAibou config"
read_when:
...
Mattermost ships as a bundled plugin in current DennouAibou releases, so normal
...
4. Configure DennouAibou and start the gateway.
...
Native slash commands are opt-in. When enabled, DennouAibou registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, DennouAibou derives one from gateway host/port + `callbackPath`.
...
- Command callbacks are validated with the per-command tokens returned by
  Mattermost when DennouAibou registers `oc_*` commands.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as DennouAibou.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to DennouAibou.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from DennouAibou, not `404`.
...
DennouAibou resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), DennouAibou sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When DennouAibou sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, DennouAibou derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and DennouAibou run on the same host/network namespace.
...
- Native slash commands return `Unauthorized: invalid command token.`: DennouAibou
...
- If `callbackUrl` is omitted and logs warn that the callback resolved to
  ...
  Mattermost runs on the same host/network namespace as DennouAibou. Set an
