...
You will need to create a new application with a bot, add the bot to your server, and pair it to DennouAibou. We recommend adding your bot to your own private server. If you don't have one yet, [create one first](https://support.discord.com/hc/en-us/articles/204849977-How-do-I-create-a-server) (choose **Create My Own > For me and my friends**).

<Steps>
  <Step title="Create a Discord application and bot">
    Go to the [Discord Developer Portal](https://discord.com/developers/applications) and click **New Application**. Name it something like "DennouAibou".

    Click **Bot** on the sidebar. Set the **Username** to whatever you call your DennouAibou agent.
  ...
  <Step title="Enable Developer Mode and collect your IDs">
    ...
    Save your **Server ID** and **User ID** alongside your Bot Token — you'll send all three to DennouAibou in the next step.
  ...
  <Step title="Allow DMs from server members">
    ...
    This lets server members (including bots) send you DMs. Keep this enabled if you want to use Discord DMs with DennouAibou. If you only plan to use guild channels, you can disable DMs after pairing.

  </Step>

  <Step title="Set your bot token securely (do not send it in chat)">
    Your Discord bot token is a secret (like a password). Set it on the machine running DennouAibou before messaging your agent.
...
```
    ...
    If DennouAibou is already running as a background service, restart it via the DennouAibou Mac app or by stopping and restarting the `openclaw gateway run` process.

  </Step>

  <Step title="Configure DennouAibou and pair">

    <Tabs>
      <Tab title="Ask your agent">
        Chat with your DennouAibou agent on any existing channel (e.g. Telegram) and tell it. If Discord is your first channel, use the CLI / config tab instead.
...
Discord forum and media channels only accept thread posts. DennouAibou supports two ways to create them:
...
DennouAibou supports Discord components v2 containers for agent messages. Use the message tool with a `components` payload. Interaction results are routed back to the agent as normal inbound messages and follow the existing Discord `replyToMode` settings.
...
- DennouAibou adds a trigger button automatically
...
<AccordionGroup>
  ...
  <Accordion title="Copy IDs">
    ...
    Prefer numeric IDs in DennouAibou config for reliable audits and probes.
...
- Commands may still be visible in Discord UI for users who are not authorized; execution still enforces DennouAibou auth and returns "not authorized".
...
<AccordionGroup>
  ...
  <Accordion title="Live stream preview">
    DennouAibou can stream draft replies by sending a temporary message and editing it as text arrives.
...
```
    ...
    enabled for Discord, DennouAibou skips the preview stream to avoid double streaming.
...
```
    ...
    - `spawnAcpSessions` is only required when DennouAibou needs to create/bind a child thread via `--thread auto|here`. It is not required for `/acp spawn ... --bind here` in the current channel.
...
DennouAibou uses Discord components v2 for exec approvals and cross-context markers. Discord message actions can also accept `components` for custom UI (advanced; requires constructing a component payload via the discord tool), while legacy `embeds` remain available but are not recommended.
...
DennouAibou can join Discord voice channels for realtime, continuous conversations. This is separate from voice message attachments.
...
- DennouAibou also watches receive decrypt failures and auto-recovers by leaving/rejoining the voice channel after repeated failures in a short window.
...
Discord voice messages show a waveform preview and require OGG/Opus audio plus metadata. DennouAibou generates the waveform automatically, but it needs `ffmpeg` and `ffprobe` available on the gateway host to inspect and convert audio files.

Requirements and constraints:

- Provide a **local file path** (URLs are rejected).
- Omit text content (Discord does not allow text + voice message in the same payload).
- Any audio format is accepted; DennouAibou converts to OGG/Opus when needed.
...
```
  ...
  <Accordion title="Voice STT drops with DecryptionFailed(...)">
    ...
    - keep DennouAibou current (`openclaw update`) so the Discord voice receive recovery logic is present
