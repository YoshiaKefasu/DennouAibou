...
read_when:
  - Setting up Zalo Personal for DennouAibou
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside DennouAibou.

> **Warning:** This is an unofficial integration and may result in account suspension/ban. Use at your own risk.

## Bundled plugin

Zalo Personal ships as a bundled plugin in current DennouAibou releases, so normal
...
- On startup, DennouAibou resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, DennouAibou stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in DennouAibou state. Example:
...
- DennouAibou sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, DennouAibou sends delivered + seen acknowledgements (best-effort).
...
- The channel now runs fully in DennouAibou without external CLI binaries.
