...
read_when:
  - You want to connect DennouAibou to LINE
...
LINE connects to DennouAibou via the LINE Messaging API. The plugin runs as a webhook
...
LINE ships as a bundled plugin in current DennouAibou releases, so normal
...
- LINE signature verification is body-dependent (HMAC over the raw body), so DennouAibou applies strict pre-auth body limits and timeout before verification.
- DennouAibou processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
