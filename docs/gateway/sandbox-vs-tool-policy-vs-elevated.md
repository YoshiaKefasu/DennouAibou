...
DennouAibou has three related (but different) controls:
...
Use the inspector to see what DennouAibou is _actually_ doing:
...
- DennouAibou validates bind sources twice: first on the normalized source path, then again after resolving through the deepest existing ancestor. Symlink-parent escapes do not bypass blocked-path or allowed-root checks.
...
- `group:openclaw`: all built-in DennouAibou tools (excludes provider plugins)
