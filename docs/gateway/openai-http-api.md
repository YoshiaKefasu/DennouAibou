...
DennouAibou’s Gateway can serve a small OpenAI-compatible Chat Completions endpoint.
...
- There is no separate non-owner/per-user tool boundary on this endpoint; once a caller passes Gateway auth here, DennouAibou treats that caller as a trusted operator for this gateway.
...
DennouAibou treats the OpenAI `model` field as an **agent target**, not a raw provider model id.
...
<AccordionGroup>
  <Accordion title="What does `/v1/models` return?">
    An DennouAibou agent-target list.
...
- `/v1/models` returns DennouAibou agent targets, not raw provider catalogs.
