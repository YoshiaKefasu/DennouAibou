---
summary: "Run DennouAibou Gateway 24/7 on an Azure Linux VM with durable state"
read_when:
  - You want DennouAibou running 24/7 on Azure with Network Security Group hardening
  - You want a production-grade, always-on DennouAibou Gateway on your own Azure Linux VM
  - You want secure administration with Azure Bastion SSH
title: "Azure"
---

# DennouAibou on Azure Linux VM

This guide sets up an Azure Linux VM with the Azure CLI, applies Network Security Group (NSG) hardening, configures Azure Bastion for SSH access, and installs DennouAibou.
...
- Install DennouAibou with the installer script
- Verify the Gateway
...
<Steps>
  ...
  <Step title="Install DennouAibou (in the VM shell)">
    ```bash
    ...
    The installer installs Node LTS and dependencies if not already present, installs DennouAibou, and launches the onboarding wizard. See [Install](/install) for details.
  ...
  <Step title="Verify the Gateway">
    ...
    Most enterprise Azure teams already have GitHub Copilot licenses, but the Copilot provider is no longer bundled with DennouAibou. We recommend configuring an OpenAI or Google API key in the DennouAibou onboarding wizard. See [Model providers](/concepts/model-providers).
...
- **Deallocate the VM** when not in use (stops compute billing; disk charges remain). The DennouAibou Gateway will not be reachable while the VM is deallocated — restart it when you need it live again:
...
- For more details on DennouAibou Azure deployment with the GitHub Copilot model provider: [DennouAibou on Azure with GitHub Copilot](https://github.com/johnsonshi/openclaw-azure-github-copilot)
