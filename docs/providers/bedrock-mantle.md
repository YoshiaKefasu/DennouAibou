---
summary: "Use Amazon Bedrock Mantle (OpenAI-compatible) models with DennouAibou"
read_when:
  - You want to use Bedrock Mantle hosted OSS models with DennouAibou
...
DennouAibou includes a bundled **Amazon Bedrock Mantle** provider that connects to
...
When `AWS_BEARER_TOKEN_BEDROCK` is set, DennouAibou uses it directly. Otherwise,
DennouAibou attempts to generate a Mantle bearer token from the AWS default
...
- DennouAibou can mint the Mantle bearer token for you from AWS SDK-compatible
