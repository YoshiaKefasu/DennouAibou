...
This document explains how DennouAibou manages sessions end-to-end:
...
DennouAibou is designed around a single **Gateway process** that owns session state.
...
DennouAibou persists sessions in two layers:
...
DennouAibou resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, DennouAibou reports potential evictions but does not mutate the store/files.
...
DennouAibou intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
When DennouAibou splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, DennouAibou
...
- If a trailing tool-result block would otherwise push the chunk over target,
  DennouAibou preserves that pending tool block and keeps the unsummarized tail
  intact.
...
These are Pi runtime semantics (DennouAibou consumes the events, but Pi decides when to compact).
...
DennouAibou also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, DennouAibou bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, DennouAibou leaves it alone.
...
DennouAibou supports “silent” turns for background tasks where the user should not see intermediate output.
...
- DennouAibou strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, DennouAibou also suppresses **draft/typing streaming** when a
...
DennouAibou uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but DennouAibou’s
