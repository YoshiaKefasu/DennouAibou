...
DennouAibou integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- DennouAibou owns lifecycle when `channels.signal.autoStart=true`.
...
- DennouAibou spawns `imsg rpc` as a child process (legacy iMessage integration).
