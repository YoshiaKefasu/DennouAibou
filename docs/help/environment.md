---
summary: "Where DennouAibou loads environment variables and the precedence order"
read_when:
...
DennouAibou pulls environment variables from multiple sources. The rule is **never override existing values**.
...
On Ubuntu fresh installs that use the default state dir, DennouAibou also treats `~/.config/openclaw/gateway.env` as a compatibility fallback after the global `.env`. If both files exist and disagree, DennouAibou keeps `~/.openclaw/.env` and prints a warning.
...
DennouAibou also injects context markers into spawned child processes:
...
- `COLORFGBG`: if your terminal exports it, DennouAibou uses the background color hint to auto-pick the TUI palette.
...
DennouAibou supports two env-driven patterns:
...
| Variable               | Purpose                                                                                                                                                                             |
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OPENCLAW_HOME`        | Override the home directory used for all internal path resolution (`~/.openclaw/`, agent dirs, sessions, credentials). Useful when running DennouAibou as a dedicated service user. |
...
On Linux, DennouAibou automatically detects nvm and applies the fix in the actual startup environment:
...
Export the variable before starting DennouAibou:
