---
summary: "Fix Chrome/Brave/Edge/Chromium CDP startup issues for DennouAibou browser control on Linux"
...
DennouAibou's browser control server fails to launch Chrome/Brave/Edge/Chromium with the error:
...
On Ubuntu (and many Linux distros), the default Chromium installation is a **snap package**. Snap's AppArmor confinement interferes with how DennouAibou spawns and monitors the browser process.
...
Then update your DennouAibou config (`~/.openclaw/openclaw.json`):
...
If you must use snap Chromium, configure DennouAibou to attach to a manually-started browser:
...
[Unit]
Description=DennouAibou Browser (Chrome CDP)
...
You're using an `existing-session` / Chrome MCP profile. DennouAibou can see local Chrome,
