...
- For shell scripts and direct interpreter/runtime file invocations, DennouAibou also tries to bind
...
- requested exec policy in DennouAibou config (`tools.exec.*`)
...
When `tools.exec.strictInlineEval=true`, DennouAibou treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
If you explicitly opt `jq` into `safeBins`, DennouAibou still rejects the `env` builtin in safe-bin
...
After an approved async exec finishes, DennouAibou sends a followup `agent` turn to the same session.
...
topics, DennouAibou preserves the topic for the approval prompt and the post-approval follow-up.
...
When an async exec approval is denied, DennouAibou prevents the agent from reusing
