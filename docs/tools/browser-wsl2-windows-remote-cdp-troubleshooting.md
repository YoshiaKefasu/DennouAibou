...
read_when:
  - Running DennouAibou Gateway in WSL2 while Chrome lives on Windows
...
- DennouAibou Gateway runs inside WSL2
...
- DennouAibou and Chrome are on the same machine
...
- DennouAibou points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, DennouAibou is not the problem yet.
...
Fix that before touching DennouAibou config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point DennouAibou at the address that is reachable from WSL2:
...
- use HTTP(S) when you want DennouAibou to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools socket URL
- test the same URL with `curl` before expecting DennouAibou to succeed
...
3. DennouAibou config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug DennouAibou config or Control UI auth
