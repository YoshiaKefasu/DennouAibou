...
- On Windows, if ACL verification is unavailable for a provider path, DennouAibou fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
