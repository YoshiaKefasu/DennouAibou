...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an DennouAibou Gateway.
...
If you want an external MCP client to talk directly to DennouAibou channel
...
- DennouAibou acts as an ACP server
- an IDE or ACP client connects to DennouAibou
- DennouAibou forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where DennouAibou runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to DennouAibou: use `openclaw acp`
- DennouAibou should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the DennouAibou gateway or agent instead.                                                                                                                                  |
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run DennouAibou through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass “yolo” switch for that harness session.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want
it to drive an DennouAibou Gateway session.
...
If you want ACPX-backed sessions to see DennouAibou plugin tools, enable the
...
DennouAibou bot over ACP, use `acpx` with its built-in `openclaw` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx openclaw` at `openclaw acp`.
3. Target the DennouAibou session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default DennouAibou ACP session
acpx openclaw exec "Summarize the active DennouAibou session state."

# Persistent named session for follow-up turns
acpx openclaw sessions ensure --name codex-bridge
acpx openclaw -s codex-bridge --cwd /path/to/repo \
  "Ask my DennouAibou work agent for recent context relevant to this repo."
```
...
For a repo-local DennouAibou checkout, use the direct CLI entrypoint instead of the
...
pull contextual information from an DennouAibou agent without scraping a terminal.
...
In Zed, open the Agent panel and select “DennouAibou ACP” to start a thread.
