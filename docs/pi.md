...
summary: "Architecture of DennouAibou's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in DennouAibou
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how DennouAibou integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.
...
├── pi-tools.ts                    # createDennouAibouCodingTools()
...
Image injection is prompt-local: DennouAibou loads image refs from the current prompt and
...
DennouAibou wraps this with `guardSessionManager()` for tool result safety.
...
DennouAibou maintains an auth profile store with multiple API keys per provider:
...
DennouAibou loads custom pi extensions for specialized behavior:
...
DennouAibou also has a local TUI mode that uses pi-tui components directly:
